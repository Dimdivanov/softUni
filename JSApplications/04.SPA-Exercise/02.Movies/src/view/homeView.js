import { html } from '../../node_modules/lit-html/lit-html.js';
import { dataService } from '../service/dataService.js';
import { userHelper } from '../utility/userHelper.js';

const homeViewTemp = (data, isLogged) => html`
  <section id="home-page" class="view-section">
    <div class="jumbotron jumbotron-fluid text-light" style="background-color: #343a40">
      <img
        src="https://slicksmovieblog.files.wordpress.com/2014/08/cropped-movie-banner-e1408372575210.jpg"
        class="img-fluid"
        alt="Responsive image"
        style="width: 150%; height: 200px"
      />
      <h1 class="display-4">Movies</h1>
      <p class="lead">Unlimited movies, TV shows, and more. Watch anywhere. Cancel anytime.</p>
    </div>

    <h1 class="text-center">Movies</h1>

    <!--add movie if logged in -->
    ${isLogged ? addMovieBtn() : ''}
    <section id="movie">
      <div class="mt-3">
        <div class="row d-flex d-wrap">
          <ul id="movies-list" class="card-deck d-flex justify-content-center">
            <!--generate movies here -->
            ${data.map((show) => generateMovies(show, isLogged))}
          </ul>
        </div>
      </div>
    </section>
  </section>
`;
function generateMovies(showData, isLogged) {
  return html`
    <!-- list item example -->
    <li class="card mb-4">
      <img class="card-img-top" src=${showData.img} alt="Card image cap" width="400" />
      <div class="card-body">
        <h4 class="card-title">${showData.title}</h4>
        <a href="#"> </a>
      </div>
      <!--details button if logged in user -->
      ${detailsForUsers(showData)}
    </li>
  `;
}

function addMovieBtn() {
  return html`
    <section id="add-movie-button" class="user">
      <a href="/addShow" class="btn btn-warning">Add Movie</a>
    </section>
  `;
}

function detailsForUsers(data) {
  return html`
    <div class="card-footer">
      <a href="/details/${data._id}" type="button" class="btn btn-info">Details</a>
    </div>
  `;
}

export async function showHomeView(ctx) {
  const data = await dataService.getAllShows();
  const isLogged = userHelper.getUserToken();
  ctx.render(homeViewTemp(data, isLogged));
}
